/// @description Bridge Collsion
	...
	var _playerBridgeCollide = collision_rectangle(pos_x_1, pos_y_1, pos_x_2, pos_y_2, Player, true, true);
		
	//Make this work if the player has collided *and* is in ground mode
	if(_playerBridgeCollide && Player.ground)
	{
		//Tension Values
		var accurate_player_x_pos = round(Player.x/LogWidth) * LogWidth;
		var player_x_pos = accurate_player_x_pos - x;
		var value_a = max(floor(player_x_pos / LogWidth), 0)
		...
		var divider = floor(accurate_player_x_pos - x-HalfWidth);
	...
	for(var i = 0; i < NumLogs; ++i)
	{
		if(instance_exists(Logs[i].LogObject))
		{
			//Variables
			...
			//Adjust the height
			Logs[i].LogObject.y = lerp(actual_height, new_height, 0.2);
		}
	}
